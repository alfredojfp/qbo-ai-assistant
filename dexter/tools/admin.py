"""dexter.tools.admin — 5 tools de administración."""
...
SCHEMA: List[Dict[str, Any]] = [
        ...
        {'type': 'function', 'function': {'name': 'gestionar_memoria', 'description': 'Gestiona la memoria persistente del agente. Usar para guardar datos aprendidos sobre Alfredo, el entorno QBO, o correcciones. Acciones: add (agregar), remove (eliminar), status (ver estado).', 'parameters': {'type': 'object', 'properties': {'target': {'type': 'string', 'enum': ['memory', 'user'], 'description': "Tipo: 'memory' (notas del agente) o 'user' (perfil de Alfredo)"}, 'action': {'type': 'string', 'enum': ['add', 'remove', 'status'], 'description': 'Acción a realizar'}, 'content': {'type': 'string', 'description': 'Texto a agregar (para action=add)'}, 'old_text': {'type': 'string', 'description': 'Substring a eliminar (para action=remove)'}}, 'required': ['target', 'action']}}},
]
